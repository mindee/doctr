...
"""Table structure recognition latency benchmark"""
...
def main(args):
    ...
    model = table_structure.__dict__[args.arch](pretrained=args.pretrained).eval().to(device=device)

    img_tensor = torch.rand((1, 3, args.size, args.size)).to(device=device)

    # Warmup
    for _ in range(5):
        _ = model(img_tensor, return_preds=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="docTR latency benchmark for table structure recognition (PyTorch)",
