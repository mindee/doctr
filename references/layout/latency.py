...
"""Layout detection latency benchmark"""
...
def main(args):
    ...
    model = layout.__dict__[args.arch](pretrained=args.pretrained).eval().to(device=device)

    # Input
    img_tensor = torch.rand((1, 3, args.size, args.size)).to(device=device)
    padding_masks = torch.zeros((1, args.size, args.size), dtype=torch.bool).to(device=device)

    # Warmup
    for _ in range(10):
        _ = model(input=img_tensor, masks=padding_masks)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="docTR latency benchmark for layout detection (PyTorch)",
