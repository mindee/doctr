...
def _fonts_per_char(
    ...
    """Map each character of the vocab to the fonts able to render it."""
...
class _CharacterGenerator(AbstractDataset):
    ...
    def _read_sample(self, index: int) -> tuple[Any, int]:
        ...
        if len(self._data) > 0:
            ...
            font = random.choice(self._fonts_per_char[char])  # nosec B311
            pil_img = synthesize_text_img(char, font_family=font)
...
class _WordGenerator(AbstractDataset):
    ...
    def _generate_string(self, min_chars: int, max_chars: int, vocab: str | None = None) -> str:
        num_chars = random.randint(min_chars, max_chars)  # nosec B311
        characters = self.vocab if vocab is None else vocab
        return "".join(random.choice(characters) for _ in range(num_chars))  # nosec B311

    def _synthesize_sample(self) -> tuple[Image.Image, str]:
        font = random.choices(self._fonts, weights=self._font_weights)[0]  # nosec B311
