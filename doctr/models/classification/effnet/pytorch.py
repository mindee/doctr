...
from torchvision.models import efficientnet_v2_m as tv_effnetv2

from doctr.datasets import VOCABS

from ...utils import load_pretrained_params

__all__ = ['efficientnet_b0', 'efficientnet_b3', 'efficientnetv2_m']
...
def _efficientnetv2(
    ...
    **kwargs: Any
    ...
    kwargs['num_classes'] = kwargs.get('num_classes', len(default_cfgs["efficientnetv2_m"]['classes']))
    kwargs['classes'] = kwargs.get('classes', default_cfgs["efficientnetv2_m"]['classes'])

    _cfg = deepcopy(default_cfgs["efficientnetv2_m"])
    _cfg['num_classes'] = kwargs['num_classes']
    _cfg['classes'] = kwargs['classes']
    kwargs.pop('classes')

    # Build the model
    model = arch(weights = "EfficientNet_V2_M_Weights.IMAGENET1K_V1")
    model.features[0][0].stride = (2, 1)
    model.features[2][0].block[1][0].stride = (2, 1)
    if kwargs.get("vert_stride", None):
        vert_stride = 1
