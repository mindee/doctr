...
from doctr.models import ResnetStage, DetectionModel,conv_sequence
...
class DecoderBlock(Sequential):
    ...
    def __init__(
        ...
        _layers = []
        _layers.extend(conv_sequence(in_chan // 4, 'relu', True, kernel_size=1, **kwargs))
        _layers.append(
        ...
        _layers.extend(conv_sequence(out_chan, 'relu', True, kernel_size=1, **kwargs))
...
class LinkNetPyramid(layers.Layer, NestedObject):
    ...
    def __init__(
        ...
        self.decoder_2 =  DecoderBlock(in_chan=64, out_chan=128)
        self.decoder_3 =  DecoderBlock(in_chan=128, out_chan=256)
        self.decoder_4 =  DecoderBlock(in_chan=256, out_chan=512)
...
class LinkNet(Sequential, NestedObject):
    ...
    def __init__(
        ...
        _layers = []
        _layers.extend(conv_sequence(64, strides=2, 'relu', True, kernel_size=7, input_shape=input_shape, **kwargs))
        ...
        _layers.append(layers.BatchNormalization())
        _layers.append(layers.Activation('relu'))
        _layers.extend(conv_sequence(32, strides=1, 'relu', True, kernel_size=3, **kwargs))
        _layers.append(
            layers.Conv2DTranspose(
                filters=out_chan,
                kernel_size=2,
...
def _linknet(arch: str, pretrained: bool, input_shape: Tuple[int, int, int] = None, **kwargs: Any) -> LinkNet:
    ...
    # Build the model
    model = LinkNet(out_chan=out_chan, input_shape=input_shape, cfg=_cfg, **kwargs)
...
def linknet(pretrained: bool = False, **kwargs: Any) -> DBNet:
    """LinkNet as described in `"LinkNet: Exploiting Encoder Representations for Efficient Semantic Segmentation"
    <https://arxiv.org/pdf/1707.03718.pdf>`_.
