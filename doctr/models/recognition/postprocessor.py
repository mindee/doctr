...
def ctc_decoder(
    ...
    """
    batch_len = tf.cast(tf.shape(logits)[0], dtype="int32")
    sequence_len = tf.cast(tf.shape(logits)[1], dtype="int32")    
    
    # computing prediction with ctc decoder
    decoded_logits = K.ctc_decode(
        tf.nn.softmax(logits), sequence_len * tf.ones(shape=(batch_len,), dtype="int32"), greedy=True
    )
    _predictions = tf.squeeze(decoded_logits[0])
    
    # masking -1 of CTC with num_classes (embedding <eos>)
    pred_shape = tf.shape(_predictions)
    mask_eos = num_classes * tf.ones(pred_shape, dtype="int32")
    mask_1 = -1 * tf.ones(pred_shape, dtype="int32")
...
def postprocessor(
    ...
    Returns:
        a tensor of tf.strings predictions

    """ 
    # decode ctc for ctc models
    predictions = decode_ctc_to_classes(logits, num_classes)

    label_mapping = label_to_idx
    label_to_idx['<eos>'] = num_classes
    label, _ = zip(*sorted(label_to_idx.items(), key=lambda x: x[1]))
    tf_label_to_idx = tf.constant(value=label, dtype=tf.string, shape=[num_classes + 1], name='dic_idx_label')
    ...
    if ignore_case: 
        decoded_strings_pred = tf.strings.lower(decoded_strings_pred)
    
    if ignore_accents:
        raise NotImplementedError

    return decoded_strings_pred
