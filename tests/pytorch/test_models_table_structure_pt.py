...
def _grid_target(rows=2, cols=3):
...
def test_table_models(arch_name, input_shape, train_mode):
    batch_size = 2
    model = table_structure.__dict__[arch_name](pretrained=True)
    ...
    target = [_grid_target(), _grid_target()]
    ...
    if not train_mode:
        ...
        for pred in out["preds"]:
            ...
            if pred["polygons"].size:
                assert pred["polygons"].shape[1:] == (4, 2)
...
def test_table_structure_zoo(arch_name):
    # Model
